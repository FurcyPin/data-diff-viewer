...
function DiffReport() {
  ...
  const selectDiffDetailRow = async (clicked_diff_detail_row, selected_diff_row, column_name) => {
    ...
    let sample_data = await report_loader.getSampleData(sampleTableNames, selected_diff_row["sample_ids"])
    let sample_rows_sorted_by_column = sample_data.map(sample_row =>
      new Map(
        diff_per_col.map(diff_col => [diff_col.column_name, sample_row.get(diff_col.column_name)])
      )
    )
...
function DataDiffReport({report_title, diff_summary, diff_per_col, on_select_diff_detail_row, sample_rows}) {
  return (
    <div>
...
function DataDiffTable({ diff_summary, diff_per_col, on_select_diff_detail_row }) {
...
function DetailsTable({col_diff, diff_summary, elements, no_change, on_select_diff_detail_row}) {
  return (
...
function ElementDetails({col_diff, diff_summary, element, on_select_diff_detail_row}) {
  if (element.count === "0") {
...
function DiffDetailRow({element, col_diff, diff_row, on_select_diff_detail_row}) {
  ...
  return (
    ...
    <tr
      ...
      key={diff_row.value}
      onClick={(e) => on_select_diff_detail_row(e.currentTarget, diff_row, col_diff.column_name)}
    >
      <React.Fragment>
    ...
    <React.Fragment>
      {
        Array.from(sample_row.entries()).map(([col_name, col_diff], id) =>
          <SampleDataRowVerticalCell key={id} col_name={col_name} col_diff={col_diff} />
        )
      }
    </React.Fragment>
  )
...
function RightDiff({char_diff, className=""}) {
  if(char_diff) {
    ...
    return (
      <td className={"RightDiff missing"}>
      </td>
    )
  }
}
